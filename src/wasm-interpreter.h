...
public:
  ...
  using TrapFunc = std::function<void(std::string_view)>;
      ...
      createTable(
        createTable != nullptr
          ...
          : static_cast<std::function<CreateTableFunc>>(
              [externalInterface](Literal initial,
                                  Table t) -> std::unique_ptr<RuntimeTable> {
                return std::make_unique<RealRuntimeTable>(
                  initial, t, [externalInterface](std::string_view s) {
                    externalInterface->trap(s);
                  });
              })) {
